...
describe("context-window snapshot dedup", () => {
  ...
  it("lets an explicit null snapshot clear stale context in the same turn", () => {
    const valid = makeContextWindowActivity("ctx-valid", 5_000, "turn-a");
    const cleared: OrchestrationThreadActivity = {
      ...makeContextWindowActivity("ctx-cleared", 0, "turn-a"),
      payload: { usedTokens: null, maxTokens: 100_000 },
    };

    const projected = projectThreadDetailSnapshot({
      snapshotSequence: 7,
      thread: makeThread([valid, cleared]),
    });

    expect(projected.thread.activities.map((activity) => activity.id)).toEqual([cleared.id]);
    expect(deriveLatestContextWindowSnapshot(projected.thread.activities)).toBeNull();
  });
